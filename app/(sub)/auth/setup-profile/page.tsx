// app/(sub)/auth/setup-profile/page.tsx
...
export default function SetupProfilePage() {
  ...
  return (
    <div className="bg-[var(--color-background)] min-h-screen flex flex-col p-5">
      <header className="w-full flex justify-end flex-shrink-0 z-10">
        <button
          onClick={handleSkip}
          className="text-sm text-[var(--text-subtle)] rounded-full px-4 py-2 hover:bg-[var(--color-subtle-bg)] active:bg-[var(--color-border)] transition-colors"
        >
          나중에 하기
        </button>
      </header>
        ...
        >
          ...
          >
            {profileImage ? (
              ...
              // ✅ [수정] 기본 프로필 이미지를 Remix Icon으로 변경
      ...
      <motion.footer
        className="w-full max-w-sm mx-auto flex-shrink-0 mt-8"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
