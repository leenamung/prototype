...
const DiaryDetailClient = ({ diary }: { diary: DiaryEntry }) => {
  ...
  return (
    <RemoveScroll className="h-full flex flex-col relative">
      <AnimatePresence>
        <div key="diary-detail-content" className="contents"> {/* AnimatePresence 직하위 요소 래핑용 (선택적) */}
          
          {/* 1. 감성 그라데이션 배경 (Absolute로 전체 덮음) */}
          <motion.div
          ...
          >
            <div className="noise-background" style={{ zIndex: 1 }} />
          </motion.div>

          {/* 2. 콘텐츠 스크롤 영역 (Flex-1) */}
          {/* overflow-y-auto를 여기에 적용하여 이 영역만 스크롤되게 함 */}
          {/* pt-20, pb-28 제거하고 적절한 내부 여백(p-4)만 적용 */}
          <div className="flex-1 overflow-y-auto relative z-10 p-4">
            <motion.div
              layoutId={`diary-card-${diary.id}`}
              className="w-full max-w-2xl mx-auto bg-[var(--color-component-bg)]/80 backdrop-blur-lg rounded-xl shadow-lg overflow-hidden border border-white/20"
          ...
          {/* 3. 하단 인터랙션 바 (Flex-none, Footer) */}
          {/* fixed 제거하고 flex 흐름의 마지막에 배치하여 항상 하단에 고정 */}
          <motion.div
            className="flex-none z-20 w-full py-4 flex justify-center bg-transparent relative"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            transition={{ delay: 0.3, type: 'spring', stiffness: 100 }}
          >
            <div className="h-14 w-fit px-4 flex items-center space-x-6 bg-[var(--color-component-bg)]/90 backdrop-blur-lg rounded-full shadow-lg border border-white/20">
