...
const DiaryCard: React.FC<DiaryCardProps> = ({ entry, optionHandle, repliySlideHandle }) => {
  ...
  return (
    <div className={`bg-[#FFFAF0] rounded-lg shadow-sm mb-4 overflow-hidden ${entry.emotionOverlayClass || ''}`}>
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center">
            <div className="w-10 h-10 rounded-full bg-[var(--color-sub-light-gray)] overflow-hidden mr-3 flex-shrink-0"> {/* 프로필 이미지 없을 시 배경색 */}
              <img 
            ...
            <div>
              <p className="font-bold text-[var(--text-main)] text-sm">{authorName}</p>
              <p className="text-xs text-[var(--text-subtle)]">{entry.timestamp}</p>
            </div>
          </div>
          <div className="relative"> {/* 팝업 메뉴의 위치 기준점 */}
            <button // div 대신 button 태그 사용 권장
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleOpenOptionsMenu(); }}
            >
        ...
        <div className="mb-3">
          {entry.type === 'image' && entry.imageUrl && (
            <div className="mb-3 rounded-md overflow-hidden">
              <img src={entry.imageUrl} alt={`${authorName}의 사진 일기`} className="w-full h-auto object-cover" />
            </div>
          )}

          {entry.type === 'video' && entry.videoInfo && (
             <div className="mb-3 rounded-md overflow-hidden relative">
                <img src={entry.videoInfo.thumbnailImage} alt={`${authorName}의 영상 일기 썸네일`} className="w-full h-auto object-cover"/>
          ...
          {entry.type === 'audio' && entry.audioInfo && (
            <div className="bg-white rounded-lg p-3 mb-3 shadow-sm"> {/* 오디오 플레이어 배경 및 그림자 추가 */}
              <div className="flex items-center mb-2">
                ...
                <span className="text-xs text-[var(--text-subtle)] ml-3">{entry.audioInfo.duration}</span>
              </div>
              <div className="w-full h-12 flex items-center justify-center">
                <img src={entry.audioInfo.waveformImage} alt="오디오 파형" className="h-full w-auto object-contain" />
              </div>
            </div>
          )}
          ...
          <p
            ref={contentRef}
            className={`text-[var(--text-main)] leading-relaxed text-sm ${isExpanded ? '' : 'line-clamp-3'}`}
          >
            {entry.content}
          </p>
          {showReadMore && (
        ...
        <div className="flex items-center space-x-4 text-[var(--text-subtle)]">
          ...
          <button // div 대신 button 태그 사용
            className="flex items-center cursor-pointer hover:text-[var(--color-primary)] transition-colors"
            aria-label="댓글 보기"
            onClick={()=>{repliySlideHandle(entry.id)}}
          >
            <div className="w-6 h-6 flex items-center justify-center mr-1">
